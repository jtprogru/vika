...
from vika.config import WX_LOCATION
from vika.services import svc_weather
from vika.utils import utils as f
...
def current(ctx):
    ...
    click.echo(
        f'\U0001F525 Temp:    {result["temp"]} - {result["min"]}/{result["max"]} (min/max)'
    )
...
def forecast(ctx):
    ...
    to_display = [
        wx
        for wx in ctx.obj.weather.forecast(location=ctx.obj.location)
        if f.is_around_midday(wx["time"])
    ]
    ...
    click.echo(
        "\U0001F4C5 Date:"
        + "".join(
            [formatter(f.convert_epoch_to(wx["time"], "%a %d %b")) for wx in to_display]
        )
    )
    click.echo(
        "\U0001F525 Temp:" + "".join([formatter(wx["temp"]) for wx in to_display])
    )
    click.echo(
        "\U0001F32A  Wind:" + "".join([formatter(wx["wind"]) for wx in to_display])
    )
    click.echo(
        "\U0001F4A7 Rain:"
